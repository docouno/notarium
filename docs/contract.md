...
Writes are compare-and-swap (#50): `versionToken` (from a read) is the proof "I'm writing over what I saw"; absent = "create / blind-append", and a stale token on an overwrite → 409 with guidance ([P3](architecture.md#p3): never lose data silently). `idempotencyKey` returns a prior attempt's result instead of a duplicate; absent = no explicit dedup, but the engine's path-upsert collapses an identical create (a keyless retry of identical content does not duplicate). A batch carries a per-item key; operations idempotent by construction (re-link) take neither.
