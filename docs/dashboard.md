...
cannot collide with either structural bucket. `None` is the compatible flat event feed.
...
clears the invalidated slice. Generic detail failures belong only to that branch;
...
graph-neutral `changed` frame and does not wait for old-generation GC.
