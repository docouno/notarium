...
- **Lazy + virtualized (#64, sized for 10k+ notes).** The tree boots from `GET /api/s/<space>/tree` (folder skeleton + counts; all space-scoped surfaces ride the `/api/s/<space>/…` prefix since #16); a folder's notes load on expand (`GET …/tree/children?path=<folder>`, shimmer skeleton rows while in flight — an un-arrived listing must not read as an empty folder). An initial listing is attempted up to three times on the bounded `FOLDER_RETRY_DELAYS_MS` schedule while one in-flight owner guards the chain. If all fetch attempts fail, the folder remains unloaded with shimmer rows until a later tree kick or manual collapse/expand; a terminal attempt invalidated by newer server truth starts a fresh bounded chain instead of losing that newer work. Background refreshes of already-held listings remain single-shot. The visible structure is flattened to rows (folders / notes / skeletons, nesting = `depth` indent) and windowed by a virtualizer over the rail's scroll pane (`VirtualTree` in Sidebar.tsx): only rows near the viewport are mounted, scrolled-past rows unmount. When a reveal intent finds the active note or folder row off-screen, it places that row at the visible top below the frosted head; once settled, later reflows use anchoring instead of revealing again.
- **Order is a view preference (#314), not cache membership.** `GET …/tree/children` carries `sort=title|created|modified` and `dir=asc|desc`; omission remains Name ascending. The one global explorer preference also drives Memory and point Favorites. Changing it re-sorts every held folder synchronously, keeps rows/scroll anchors mounted, invalidates responses issued under the old order, then reconciles those same windows in the background. SSE refresh and optimistic move use the active order too; neither may fall back to title or clear a held listing into shimmer rows.
...
- **First load expands the top level by union, once (#98 item 2).** The first tree skeleton
  unions every top-level path with the current open set, preserving any ancestor chain that
  reveal already opened. It then latches for the session, so a later *Collapse all* stays
  collapsed instead of re-seeding the roots.
