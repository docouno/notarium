...
- Since #64 the Feed owns its own **server-windowed** data: `GET /api/s/<space>/notes?sort=&offset=&limit=&folder=&depth=&folders=&tags=&field=&fieldDay=&fieldAny=&fieldBad=&q=&from=&to=&tz=&dateField=` (space-scoped since #16) computes filter+sort+slice from the read-model snapshot and returns the honest `total`; the client holds a sparse page cache (50/page, LRU 16 pages) — the full list never crosses the wire. The Feed's folder facet (#93/#109) sends `folders` (one repeated query key per selected subtree → an array server-side; INCLUSION, OR/union, subtree-cascading prefix-match, applied after `folder`); "show only" is just a one-element set. The bucket histogram takes the same filter axes so its `total` stays in lockstep with the window's. Each note carries `modifiedAt` and `createdAt` — ISO instants since #54 (`modifiedAt` is journal-precise for everything that happened on the server's watch and midnight-UTC where only the engine's day signal exists; `createdAt` is the registry's first-seen value until a valid authored `created:` SET is proven through the UI/write path or an exact external-file observation; null = the engine honestly doesn't know, and `sort=created` excludes undated notes from the window AND its total). Freshness rides the shared SSE stream; external date reconciliation adds no private event — same-day date-only edits use the journal refresh, while mixed/modified-day edits retain the existing generic plus possible journal follow-up, all after the new date is durable: a `changed` event touching a VISIBLE note — one shown under the selected-folder filter (`folderShown`) — refetches the held pages, coalesced at 1s. Relevance is read on TWO axes, old ∪ current: where this session last placed the `upserts`/`removed` ids (its own resolution cache) AND the server-truth `folders` the event carries. The union is what makes an EXTERNAL move visible: when another client or an agent moves a note INTO the selection, our cache still holds only the folder it left, so the old axis alone would drop the event as irrelevant and the window would stay stale until a filter flip, a reconnect or a reload (the reverse move never had the problem — the old axis sees it leave). Precision survives: `folders` are the upserted notes' own folders, so a move between two unselected folders still refetches nothing. Independently, an `upserts` id matching the open detail single-flights one in-place detail refresh, so Meta/header/token do not wait for a 409. Folder facets + mini-stats come from `GET /api/s/<space>/tree` via NotesProvider.
...
- **Active-detail SSE coalescing.** A detail attempt that started before an active-note `upserts` event is discarded and retried inside the same task. If a post-event refresh has already advanced the open note's version token before the coalesced timer fires, the timer does not repeat that GET.
- `GET /api/s/<space>/notes/buckets?sort=&group=day|week|month&folder=&depth=&tags=&field=&fieldDay=&fieldAny=&fieldBad=&q=&from=&to=&tz=&dateField=` → the **date histogram** of a notes query: consecutive `(key, count)` runs in list order (key = bucket start as a local YYYY-MM-DD; `''` = the trailing undated bucket), counts summing to the same `total` the matching `/api/notes` reports. `tz` is the client's UTC offset in minutes east, so day boundaries match the user's clock. This is what grouped layouts are built FROM — headers, section sizes and the honest scrollbar are known before a single item loads.
- **Field schema carrier (#384 V04).** A space-scoped `FieldSchemaProvider` reads the durable declaration document and caches it per space. Its opaque file version token joins the Feed `queryKey`: a successful schema PUT invalidates the sparse window and refetches the field facet. The provider sits above `FeedProvider`; card plaques consume the same declaration later.
- **Declared card fields (#384 V06) do not use previews.** The notes window carries primary `noteType` independently from the bounded custom-fields blob, plus a compact `fields` map containing only declarations marked `card: true`; enum display labels/tone and declaration order come from the space-scoped field-schema provider while the window keeps the stable stored key. Custom dates display their authored calendar prefix through `fieldDate`; Created remains an instant. Card metadata is date → Type → value-only fields → Tags. Every present marked value renders in schema order; Type and fields subtract from the four-chip tag budget, but fields have no separate cap. Tight layouts clip overflow. A mismatched value stays neutral. The note detail header mirrors Created → Type → value-only fields → Tags. Tree children and Favorites call the same wire mapper without the requested field-key list and therefore carry no `fields` payload.
- **View summaries are a separate primary-view batch** ([views canon](views.md#discovery)). Feed windows set `viewSummary=1`; ordinary notes/tree calls do not. Marker-bearing rows are evaluated at most 100 per request over one source snapshot. A ready summary replaces the generic snippet immediately (`M columns · K cards`, with the `N views · primary:` prefix only for multiple views). An unavailable/marker-only result is omitted from the list wire, so the card falls back to the ordinary semantic-prose preview; `View unavailable` and raw `nota` YAML are never Feed copy. Preview fetching still supplies prose/image/tags and remains independently cached/invalidation-driven.
- **Field filter axis (#384 V02).** Four repeatable parameters share one grammar: `field=note.<key>:<value>` (exact scalar equality or list containment), `fieldDay=note.<key>:YYYY-MM-DD` (one authored calendar day on a declared `date`, matching valid day-only and instant spellings), `fieldAny=note.<key>` (the key is indexed in any readable/unreadable/truncated form), and `fieldBad=note.<key>` (the value is unreadable). Conditions for one key are OR; different keys are AND. Namespace `note.` is mandatory; `file.`, unknown namespaces and projected keys such as `tags` are actionable 400s. A colon belongs to the value after the first separator; the entire `field` RHS stays an opaque exact literal, so `>3` and `[a..b]` are ordinary strings rather than comparison syntax. Exact equality is byte-exact — no tag folding, trim or Unicode normalization — while `fieldDay` first requires a valid declared-date value and then compares its authored prefix.

  `GET /api/s/<space>/fields` is the filter's corpus facet. It is filter-independent like `/tags`: input is key-prefix `q`, a bounded key `limit` (default 24) and `valuesLimit` (default/max 100; Settings uses 0 when it needs only usage counts). Every declared key is eligible. Undeclared discovery keeps a deterministic lexical window before stats/sort; a truncated response says so, its root `total` is then only the retained eligible count, and `q` applies before the window so an omitted key remains directly discoverable. Undeclared high-cardinality identity values saturate out instead of retaining their whole vocabulary. Protected keys and keys containing a colon never appear. Each row carries `notes`, stable stored values with counts, and its pre-slice `total`. Declared enum keys follow schema order (including zero counts), then observed out-of-catalog values; other values sort by count then value. Date day/instant spellings with one authored prefix form one chip whose click writes one bounded `fieldDay`, never an OR expansion of raw literals. The browser displays the option label but keeps its key in the URL and query. `sort=created` may be lower because it excludes undated notes while the facet does not.

  Prefix `q` is an API discovery seam, not a current Feed control. When bounded discovery reports `truncated`, the Feed gives a passive incomplete-list notice; technical-key search is a separate UI feature.

  The browser keeps all four forms as repeated URL state. Every field is its own facet section beside Tags — there is no intermediate “Fields” group. Tags and fields share the interactive `core/FacetChip`: one geometry and hover/active/focus language, optional semantic color, optional icon (`#` for tags, none for fields), and a keyboard-reachable trailing action for the tag hierarchy. A field section reset clears equality/day/presence/unreadable forms only for that key. A chip click updates the URL, `queryKey`, window and buckets together. Corpus SSE/schema changes refetch the facet only while Feed or Fields Settings owns it; off-route changes do no corpus-wide field reduction. A cold Feed waits for schema identity before its first notes window.
- **Tag axis (#109/#204).** Tags are a server-side navigation axis, not just card chips. `GET /api/s/<space>/tags` is the **tag facet**: every tag with note counts (`{tag, label, count, direct}`, flat-sorted-by-path → the client nests `a/b`), computed from the read-model snapshot under the same class-visibility checkpoint as `/tree`. The shaper is shared — `core/libs/tags` `buildTagFacet` — so the server endpoint and the graph (which builds the same shape client-side off `GraphRealNode.tags`) agree by construction. `/api/notes` (and `/notes/buckets`) take a repeated `tags=` query key (an **OR/union set**, hierarchical + case-insensitive via `foldTag`/`matchesTags` — `?tags=ml` also lists `ml/nlp`; multiple keys widen the result), so the window/`total`/histogram all describe the tag-filtered population — no client-side preview sweep. The selected tags are **URL state** (a repeatable `?tag=`): `useFeedState` reads `getAll('tag')` (drives the window + histogram + `queryKey`); `toggleTag`/`clearTags` edit the set. **UI — one shared pane, one filter language.** The `TagFilter` widget (used identically in `FeedAside` and the Graph filters) is **hierarchical chips + search + top-N**: inclusion (nothing selected = no filter, a click ADDS a tag — accented), a parent's `▾` reveals its children as an indented sub-row, search jumps to any tag. The active filter reads off the aside chips — **no chip floats over the Feed content**. Read-surface tags render through `core/Chips` `TagChip`/`TagChips`: neutral by default and future-color-ready through the shared `solid / fg / surface / border` tone channels. Reader and Meta chips link to `/s/<space>/feed?tag=<folded>`; Feed-card chips are not links because the whole card is already the note link. The graph adds a tri-state above the same filter chips; specific-tag selection is OR + hierarchical over `GraphRealNode.tags`.
...
- `useFeedState` (`packages/web/src/composers/FeedProvider/hooks/useFeedState/useFeedState.ts`) is **lifted** state shared by the page and the
  aside (one data window). Owns: `sort`, `view`, `cols`, `group`, the selected-folder
  set (`selected` + `toggleFolder`/`soloFolder`/`resetFolders`, #93 — inclusion, so
  empty means no filter and a selection cascades over its subtree),
  the sparse window (`total`, `itemAt`, `ensureRange`), the URL filter axes
  (`tag`, `field`, `fieldDay`, `fieldAny`, `fieldBad`, `q`, `from`, `to`), the histogram (`buckets`, refetched with the pages on
  the coalesced SSE sweep), `folders`, `stats`.
  Persists prefs to localStorage (the selected set is NOT persisted — graph parity).
...
- `styles.css` (timeline): `--tl-gutter: 160px`; columns `padding-top: 8px` + date/
  ...
  `var(--border)`.
- Chips: `.feed-tag` is the Feed's contextual visibility marker on both shared neutral
  `TagChip` read chips and declared field `Chip`s; this is what hides both in c5.
  `.feed-date` is a neutral grey date pill. Grid order = date, Type, fields, then tags
  (bottom-left), with fields consuming tag slots from the fixed budget of four. In the timeline the date is a plain grey
...
- A card renders date first, then Type, every present schema-marked value-only field in schema
  order, then the tags that fit the remaining four-chip tag budget; mismatches have no semantic
  color and the field list itself is never sliced.
