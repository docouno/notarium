...
- **Published JSON Schema and runtime parsing are two views of one contract.** `tools/list` receives a representable open-record shape for `get_note.frontmatter` and `edit_note.fields`; the SDK validates with the runtime Zod schema behind that published view. This preserves own prototype-shaped keys on read, rejects unsafe write keys before the handler, and never degrades discovery to `{}` or an unrepresentable transform.
- **Endpoint:** `POST /mcp` — one stateless endpoint for both protocol eras, backed by the official MCP packages and a fresh server per request scoped to the token. A 2026-07-28 client starts with `server/discover`; a 2025 client keeps the `initialize` handshake. Both paths return one JSON response (not SSE), ignore `Mcp-Session-Id`, and expose the same gateway. `GET`/`DELETE` → `405`. Subscriptions are not supported: a conformant 2026 `subscriptions/listen` receives `404/-32601`, while the legacy path retains its protocol-level `200/-32601`. Modern `tools/list` and `server/discover` results carry private five-minute cache hints; legacy results do not.
...
- **Server instructions** are returned by `server/discover` in the 2026 era and by `initialize` in the 2025 era (the main lever for call order). They are static text; note content is **never** mixed into them (anti tool-poisoning).
...
- `whoami` — who I am and what I may do (principal-id, the read|write ceiling, project memberships), the engine's `capabilities` (`vector`/`trash`/`revisions`) and the separate principal-specific `hasModel` boolean. `hasModel` is always present; it is true only when the principal has an effective available model/capability pair, and false when providers are disabled, resources are empty, or every pair is unavailable. The response never exposes provider names, model names or statuses ([provider boundary](providers.md#enable-and-access)).
...
**Discover / navigate**
- `list_notes` — `ls` of the knowledge base: the direct notes and subfolders of a folder (deterministic, paginated). `project` picks a space and narrows to it; `path` is a **space-relative** folder (take the `path` from a `folders` entry / a hit verbatim, do not construct it) — without it, the project root. `tag` filters by a tag. Four repeatable field filters use the Feed grammar: `field: ['note.<key>:<value>']` for exact equality/list containment, `fieldDay: ['note.<key>:YYYY-MM-DD']` for an authored day on a declared date, `fieldAny: ['note.<key>']` for presence in any indexed form, and `fieldBad: ['note.<key>']` for an unreadable value. Conditions for one key are OR and different keys are AND; malformed namespaces/projected keys and `fieldDay` on a non-date declaration fail with a reason. The equality RHS stays an opaque literal, including `>3` and `[a..b]`. Returns direct `items`, direct `folders`, `total` and the listed folder's own **Folder page** slot; page with `cursor`/`nextCursor`. Lists user-visible notes; the filter applies only to the direct-note window. `folderPage` is `{status:'present', folderPath, folderId?, noteId, title}` or `{status:'missing', folderPath, folderId?, createWith?}` — and it is absent entirely when the path holds no folder (a ghost path still answers with today's empty listing, never an invented folder to author into). The page is the folder's **cover, not a child**: it is lifted out of the population before any filter, cursor or `total`, so the slot reads identically on every page of a filtered listing ([folder-page.md](folder-page.md#agent-surface-mcp-415)).
...
- `get_note` — a full note by ref (a note-id from `search`/`recall` or an in-space wiki-ref). Returns the raw content, frontmatter, `path` (where it lives — for talking to a human in folder terms; reference it by id anyway), `class` (a read-only label for the model), `versionToken` (return it in `edit_note`/`create_note`/`remember_*` for a safe write) and `provenance` (who/how/when edited it). `folderPage` (`{folderPath, folderId?}`) is present only when the note IS a folder's page — the structural role neither `class` nor `path` can express. `unsafeFrontmatterKeysOmitted` is present only when pseudo-control-shaped raw keys were withheld from the agent view; ordinary keys remain exact. In `detailed` (the default) also: `outline` (the headings — the menu of valid `section` values for `edit_note.replaceSection`), `links`, and for a view document a bounded semantic `views` projection in authored order. Raw `nota` YAML remains only in structured `content`; the human prose replaces each fence with sanitized reader prose. Column/card values keep stable raw `value` separately from label/color, exact-fallback incompleteness is explicit, and at most 50 rows total are allocated primary-first across the whole response.

Search and `list_notes` rows additionally expose discovery-only `viewType` when present. It is a
hint for choosing a document, not permission to infer board state; only `get_note.views` is the
validated semantic projection. Generic `edit_note` text operations may change surrounding prose,
but refuse when their computed before/after `nota` carrier blocks differ and direct the caller to
`edit_note.view`. Manual ranks remain unavailable to agents.
...
- `edit_note` — edit an existing note incrementally (prefer this over a full rewrite). A call carries a word-addressed body operation, a `fields` point patch, both of those, or one structural `view` patch; structural view edits do not combine with the other two. The five body modes are **by words, not by positions** (#102 phase 3): `append`/`prepend`, whole-body `replace`, heading-based `replaceSection` (menu: `get_note.outline`), and exact-unique `findReplace` (**empty `content` deletes the snippet**). `operation` and `content` must arrive together. `fields` maps an authored frontmatter key to a scalar/list, or to `null` to remove the key; `''` and `[]` remain explicit empty values. `view` addresses a fresh opaque `viewRef` and patches only source/common/options nodes with the same CAS, resource limits and comment/unknown-key preservation as the document writer; rank overlays are deliberately not agent-editable. Undeclared keys are accepted, while typed/storage-owned keys and Agent Skill root `name`/`description` are refused with an actionable error. A technical key containing an MCP pseudo-control tag is agent-inaccessible and rejected without reflecting it; the registry input schema applies the same predicate as the output sanitizer before nested field-value validation, so the SDK cannot echo the key from a validation path. Human REST/Markdown remain open-world. **Memory is just a note:** correct/delete one fact by its memory note id with the same modes. A supplied `versionToken` is enforced (CAS); without one the operation writes under the token from its own read. A body edit echoes `path` plus `bodyBytes`/`bodyHash`; a fields-only/view edit does not claim to have rewritten body bytes.
...
- `move_note` — move a note to another folder, **keeping its name**. `ref` — a note-id; `toFolder` — the **space-relative** destination folder: take the folder's `path` as `list_notes` returns it (do NOT construct the path by hand), `''` — the space root; a missing folder is created. The id and the `/n/<id>` URL are stable, **inbound links do not break** (a link by title does not depend on the folder) — only the location changes. The note stays in its space. Because the NAME travels with the note, moving a folder page makes it the destination's cover: that is allowed, because a plain `mv` on disk does the same thing and the API is not stricter than the filesystem, and where the host keeps a folder registry the destination adopts it through the page lifecycle (its folder identity is minted from the move's `finalize` — a marker describes a folder that exists, and the destination may be one this very move created — and the `always-load` pin follows where the page ARRIVED — an active project's root pins it, leaving one without landing in another releases it) — only landing on a folder that ALREADY has a page is refused, before the move. To change the title — `rename_note`, the body — `edit_note`, to move a whole **folder** — `move_folder`. Idempotent: a move to where the note already sits is a no-op.
- `rename_note` — change a note's **title** (which also drives the file name). `ref` — a note-id, `title` — the new title. The id and URL are stable; **link-safe**: the old title goes into the alias history (#100), so inbound `[[Old Title]]` keep resolving — **the bodies of linking notes are not touched** (the resolver works, not a rewrite). `versionToken` is **not needed** — the tool reads the note itself; if it is edited in parallel — an honest conflict error, retry. A new title whose resolved basename would be the reserved `index` is refused — a rename recomputes the path, so that would turn an ordinary note into its folder's page behind the page lifecycle; a note that already IS a page renames freely and keeps `index.md`. The rename refusal says to pick another title rather than to author a page. The refusal is about VISIBLE notes: a hidden-class note (agent memory, a profile) lives in a mount of its own where `index.md` is nobody's cover, and its title is not restricted. The folder is changed by `move_note`, the body by `edit_note`, the whole note is removed by `delete_note`.
...
- **Folder pages are a role, not a tool.** Browse a folder with `list_notes` and read its `folderPage`: present → `get_note` the returned `noteId`, missing → author one ONLY if the user asked for folder-level content, by passing `createWith` unchanged to `create_note` plus a `body`. From there it is an ordinary note: `get_note` marks it, `edit_note` changes it under the usual CAS, and its storage name stays `index.md`. **Compatibility correction (#415):** a folder page no longer appears as an ordinary `index` item in `list_notes`, and is no longer counted in `total` (nor in `start_session`'s `index.noteCount`) — it was always the cover in the domain tree contract, and two competing truths in one listing would be worse than one corrected one. The second correction is the reserved-name refusal above. **Moving** a page is the one door left open on purpose: `move_note` keeps the file name, so a moved page becomes its destination's cover and, where a folder registry exists, that folder adopts it through the same lifecycle (identity minted, an active project's root pinned) — a plain `mv` on disk does the same thing, and the API is not stricter than the filesystem. Only landing on a folder that ALREADY has a page is refused.
...
- **Untrusted content is defanged on output.** Title/snippet/content/frontmatter values are sanitized before being handed to the agent; pseudo-control tags are neutralized. A frontmatter key with such a tag cannot be safely renamed while remaining an exact address, so `get_note` omits that entry and reports `unsafeFrontmatterKeysOmitted`; MCP field write/filter reject the same key without reflection. Human REST/UI/Markdown keep it exact. Note content is **never** reflected into tool descriptions / the server `instructions`.
