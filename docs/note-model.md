...
Every surface that matches names calls those two: the link index and `resolveLink`, both
engines' reference resolvers, the read-model's snapshot filter, the alias history, the
client's `resolveWiki`, the memory-category lookup and the typed-link idempotency check.
That list is the point — the rule lived as six separate copies, and each review round
found another one left behind, until the copies were replaced by the primitive.
...
channel and normalizes before calling `read`, while internal inventory/body reads pass the
literal full path. Thus a legal external `Foo#section.md` remains readable as storage, but
...
different notes. Its callers are `resolveLink`'s ghost target, the fake resolver's, and
the typed-link idempotency check — where a merge is silent data loss, since the second
