...
**Chunks are cut by product domain, not by size.** `build.rollupOptions.output.manualChunks` pulls the synchronous heavyweight vendor families out of the application entry into `editor` (CodeMirror/Lezer core), `graph` (the force-graph/graphology stack shared by the full graph page and the note inspector), `markdown-math` (KaTeX), `syntax-highlighting` (highlight.js), and `view-carrier` (the YAML CST runtime shared by live/draft view documents). These are all still *static* imports — this is delivery and cache topology, nothing became lazy and no Suspense boundary was added. Every lazy surface stays lazy: the Mermaid/Cytoscape diagram families, the CodeMirror grammars `@codemirror/language-data` loads on demand, and the lazy Agents routes are all still their own dynamic chunks.
