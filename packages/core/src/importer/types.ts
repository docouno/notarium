 ...
 *  the usual create channels; `createdAt`/`fileName` ride the host-internal write
 *  channels. Provenance is carried by `tags` (e.g. `claude`); the parser
 *  derives the deterministic `fileName` from the source id internally. */
export type ImportNote = {
  title: string
  /** The note body as markdown — NO leading `# title` heading (the write path
   *  adds the storage-format heading) and NO frontmatter. */
  body: string
