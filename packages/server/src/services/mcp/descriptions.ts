...
export const TOOL_META = {
  ...
  list_notes: {
    description:
      'List the notes and subfolders directly inside a folder — an `ls` for the knowledge base, deterministic and paginated. Use it to see what is actually in a place (dedup before writing, "show me folder X", browse a project) rather than guessing with search. `project` (a handle from get_my_projects) picks the workspace and scopes to it; `path` is a SPACE-relative folder — copy one verbatim from a `folders` entry or a hit\'s `path` to drill in (do not construct paths). Omit `path` to list the project (or personal-domain) root. `tag` keeps only notes carrying that tag. `field`, `fieldDay`, `fieldAny`, and `fieldBad` apply the same repeatable field grammar as the Feed: equality/list containment (`note.<key>:<value>`), an authored calendar day on a declared date (`note.<key>:YYYY-MM-DD`), presence (`note.<key>`), and unreadable value (`note.<key>`); conditions for one key are OR and different keys are AND. Returns `items` (the folder\'s direct notes, with their id/title/path/tags and discovery-only `viewType` when present) and `folders` (its direct subfolders, each with a subtree note count), plus an honest `total`; page further with the `cursor`/`nextCursor`. It also returns `folderPage` — the folder\'s own authored page (its cover). `status:"present"` gives the `noteId` to read with get_note; `status:"missing"` means the folder has none, and its `createWith` (when present) is the exact create_note arguments that would author one — pass them unchanged and add a `body`, but only when the user actually asked for folder-level content. No `folderPage` at all means the path is not a folder here — an empty listing for a place that does not exist, not a folder still lacking its cover. The page is never one of `items` and never counted in `total`, so it reads the same on every filtered or paged listing. Lists your user-visible notes (not agent-memory — use search for that).',
    annotations: {
  ...
  get_note: {
    description:
      "Read a full note by its id (the `noteId` from search). Returns the raw content, frontmatter, `path` (where it lives — for speaking folders with a human; you still reference notes by id, not path), and a `versionToken` — pass that token back to edit_note to write safely without clobbering a concurrent edit. A pseudo-control-shaped raw frontmatter key is withheld from the agent view and counted in `unsafeFrontmatterKeysOmitted`; ordinary keys remain exact. A detailed read (the default) also returns `outline`, graph `links`, and — for a view document — semantic `views` with opaque `viewRef`, totals/groups, stable raw values plus labels/colors, explicit incomplete/truncation state, and at most 50 rows total. Human-readable prose replaces raw `nota` fences with sanitized reader output; the raw carrier remains only in structured `content`. Also returns `provenance` — who last edited it (a person or an agent), how, and when, and `folderPage` when this note IS a folder's page (its authored cover) — with the folder it covers. Identifiers are note-ids, stable across rename and move.",
    annotations: {
      title: 'Read a note',
      readOnlyHint: true,
      idempotentHint: true,
  ...
  edit_note: {
    description:
      "Change an existing note incrementally — prefer this over rewriting a whole note with create_note. `ref` is a note-id (from search or get_note). You may send a word-addressed body operation, a `fields` point patch, both of those, or one structural `view` patch; a view patch is not combined with body/fields. `view` addresses the fresh opaque `viewRef` from get_note and patches source/common/options while preserving comments and unknown keys; manual ranks are not agent-editable. Generic text may change surrounding prose but refuses when its computed before/after `nota` blocks differ, with guidance to use `edit_note.view`. `null` removes a field while `''` keeps an empty value. A pseudo-control-shaped technical field key is unavailable through MCP and is rejected without reflection. Body operations: `append`/`prepend`, whole-body `replace`, heading-based `replaceSection`, and exact UNIQUE `findReplace`. `operation` and `content` must arrive together. Memory is just a note: correct or remove a fact with findReplace (empty replacement deletes it). Pass the `versionToken` from get_note to prevent clobbering a concurrent edit. Edits keep the note where it lives and never change its kind.",
    annotations: {
      title: 'Edit a note',
      readOnlyHint: false,
      destructiveHint: true,
  ...
  move_note: {
    description:
      "Move a note to a different folder, keeping its name. `ref` is a note-id (from search/list_notes/get_note); `toFolder` is the destination folder — pass a folder `path` exactly as list_notes reports it (do NOT build a path by hand), or an empty string for the space root. A folder that does not exist yet is created. The note keeps its id and its URL, and its inbound links keep working — only where it lives changes. The note stays in its own space. Because the file name travels with the note, moving a folder page (`index`) makes it the cover of the folder it lands in — that is allowed, and where this host keeps a folder registry that folder adopts it (its id is minted, and landing in an active project's root pins it into that project's startup context — leaving one without landing in another releases that pin); moving it onto a folder that already has a page is refused. To change the TITLE instead, use rename_note; to change the body, use edit_note; to move a whole FOLDER, use move_folder. Safe to repeat: moving a note to where it already is does nothing.",
    annotations: {
      title: 'Move a note',
      readOnlyHint: false,
      destructiveHint: false,
  ...
  link: {
    description:
      'Create a typed link from one note to another so they connect in the graph. `from` is a note-id (from search or get_note); the target is EITHER `to` (an existing note-id) OR `toTitle` (a forward-reference by title — link to a note you have NOT created yet; it resolves automatically once a note with that title exists, so a staged migration never loses an edge). An existing `to` is stored as a stable identity link with a readable alias; bracket/HTML alias delimiters are safely encoded and render as the exact title. Forward `toTitle` keeps human-name semantics and does not accept a bracket title. `relation` is a short label for how they relate (e.g. "depends_on", "relates_to"). Both notes must end up in the SAME space — cross-space links are not supported yet. Idempotent: linking the same pair with the same relation again does nothing. To create MANY links at once use link_many.',
    annotations: {
  ...
  create_notes: {
    description:
      'Create SEVERAL knowledge notes in one project in a single call — the tool for migrating or importing many notes (dozens of create_note calls otherwise). `project` applies to every note; `notes` is an array, each item exactly like create_note (body — whose leading `# H1` titles the note; optional title/path/type/tags/links/createdAt/fileName/idempotencyKey) but WITHOUT its own project and WITHOUT `folderPage` (a folder page is one deliberate create_note, never a batch item; an item whose name would resolve to the reserved `index` fails on its own and leaves its siblings alone). Best-effort and NON-transactional: each note succeeds or fails independently — the response `results` array reports each by its `index` and `title` with either `ok:true` (plus the same echo create_note gives) or `ok:false` with an `error`, so you retry ONLY the failures. Inline `links` may forward-reference notes created later in the SAME batch (by `toTitle`). Search first to avoid duplicates; a per-note title collision fails just that item.',
    annotations: {
...
export const SERVER_INSTRUCTIONS = [
  ...
  "Search before you write: `search` finds existing notes — and now your own agent-memory too — so you do not create duplicates. To browse structure use `list_notes` (an `ls` of a folder) and `recent_activity` (the latest changes); a folder's own authored page rides `list_notes` as `folderPage`, never as one of its notes.",
  ...
  'Tool argument objects are closed: send only fields published in tools/list. An unknown field is an invalid-arguments tool error, and the tool does not run.',
