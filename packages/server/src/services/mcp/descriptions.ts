...
export const TOOL_META = {
  ...
  get_note: {
    description:
      'Read a full note by its id (the `noteId` from search). Returns the raw content, frontmatter, `path` (where it lives — for speaking folders with a human; you still reference notes by id, not path), and a `versionToken` — pass that token back to edit_note to write safely without clobbering a concurrent edit. A pseudo-control-shaped raw frontmatter key is withheld from the agent view and counted in `unsafeFrontmatterKeysOmitted`; ordinary keys remain exact. A detailed read (the default) also returns `outline`, graph `links`, and — for a view document — semantic `views` with opaque `viewRef`, totals/groups, stable raw values plus labels/colors, explicit incomplete/truncation state, and at most 50 rows total. Human-readable prose replaces raw `nota` fences with sanitized reader output; the raw carrier remains only in structured `content`. Also returns `provenance` — who last edited it (a person or an agent), how, and when. Identifiers are note-ids, stable across rename and move.',
    annotations: {
      title: 'Read a note',
      readOnlyHint: true,
      idempotentHint: true,
