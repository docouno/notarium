...
CREATE FUNCTION enforce_revision_append_lifecycle()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE TRIGGER trg_revision_append_lifecycle
  ...
  FOR EACH ROW
  EXECUTE FUNCTION enforce_revision_append_lifecycle();

CREATE FUNCTION enforce_revision_purge_protocol()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF current_setting('notarium.revision_purge_protocol', true) IS DISTINCT FROM 'v26' THEN
...
CREATE TRIGGER trg_revision_purge_protocol
  BEFORE DELETE ON note_revisions
  FOR EACH STATEMENT
  EXECUTE FUNCTION enforce_revision_purge_protocol();
