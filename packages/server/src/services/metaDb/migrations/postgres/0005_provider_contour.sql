...
CREATE TABLE secret_keyring (
  key_id      TEXT PRIMARY KEY CHECK (key_id ~ '^ck_[0-9a-f]{24}$'),
...
CREATE INDEX idx_credentials_owner_page
  ON credentials(owner, name COLLATE "C", id COLLATE "C");
...
CREATE INDEX idx_provider_resources_owner_page
  ON provider_resources(owner, name COLLATE "C", id COLLATE "C");
CREATE INDEX idx_provider_resources_effective_page
  ON provider_resources(name COLLATE "C", id COLLATE "C");
...
-- No FK to spaces: INSERT/UPDATE would take an implicit FOR KEY SHARE from the
-- bottom of the provider contour while purgeSpace holds that row from above.
-- The explicit lifecycle fence + provider-ordered purge carry the same invariant
-- without an unobservable lock inversion. canon: docs/meta-db.md#source-of-truth
CREATE INDEX idx_provider_attachments_target_space
  ON provider_attachments(target_space, created_at COLLATE "C", id COLLATE "C");
