 ...
 *  Owns their refs, their measured live heights (both conditional, their content
 *  varies) and the scroll-aware glass wiring — the list reserves the measured heights
 *  as padding so the first/last rows clear the frosted bars, the same way the rail
 *  head is measured. `structure` flags (which bars are mounted) re-run the measure. */
export const useTrashChrome = (
  ...
  // scrolls UNDER them. Measure their live heights (both are conditional and their
  // content varies) and reserve them as the scroll's padding, the same way the rail
  // head is measured (--panel-head-h), so the first/last rows clear the frosted bars.
  ...
  useScrollGlass(scrollRef, footerRef, { edge: 'bottom' })
